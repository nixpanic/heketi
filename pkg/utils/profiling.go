...
func EnableProfiling(router *mux.Router) {
	...
	// The index handler only lists the runtime_pprof.Profiles(), so let's not index any.
	//router.Methods("GET").Path(basePath).Name("index").Handler(http.HandlerFunc(pprof.Index))
}
